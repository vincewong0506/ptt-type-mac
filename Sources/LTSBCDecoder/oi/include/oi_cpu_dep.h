...
typedef signed char     OI_INT8;   /**< 8-bit signed integer values use native signed character data type for ARM7 processor. */
typedef signed short    OI_INT16;  /**< 16-bit signed integer values use native signed short integer data type for ARM7 processor. */
/* Local patch: the original typedefs use `signed long` / `unsigned long`,
 * which is 32-bit on ARM7 / ESP32 xtensa (the codec's intended targets) but
 * 64-bit under the iOS ARM64 LP64 ABI. The codec's bit-twiddling assumes
 * exactly 32-bit OI_INT32/OI_UINT32 (e.g. SIMD-style `n & 0x7F7F7F7F`,
 * `sizeof(OI_INT32) * subbands * blocks` buffer maths). Pinning these to
 * `int` / `unsigned int` keeps the contract on both 32-bit and 64-bit
 * compilers (ILP32 and LP64 alike). */
typedef signed int      OI_INT32;  /**< 32-bit signed integer values use native signed int data type for ARM7 processor. */
typedef unsigned char   OI_UINT8;  /**< 8-bit unsigned integer values use native unsigned character data type for ARM7 processor. */
typedef unsigned short  OI_UINT16; /**< 16-bit unsigned integer values use native unsigned short integer data type for ARM7 processor. */
typedef unsigned int    OI_UINT32; /**< 32-bit unsigned integer values use native unsigned int data type for ARM7 processor. */
